...
export default function BulkAssignButton({
  ...
  const isOwnerMode = !restrictToUnassignedOnly;
  ...
  const participantOptions = useMemo(
    () =>
      buildParticipantOptions(
        ...
        { includeAll: isOwnerMode }
      ),
    [effectiveParticipants, t, isOwnerMode]
  );

  function handleSelectParticipant(selectedValue: string) {
    if (selectedValue === ALL_PARTICIPANTS_VALUE) {
      const allIds = effectiveItems.map((i) => i.itemId);
      onAssign(allIds, ALL_PARTICIPANTS_VALUE);
      return;
    }

    const selected = participantOptions.find((p) => p.value === selectedValue);
    if (!selected) return;
      ...
      <Menu as="div" className="relative">
        ...
        >
          ...
          {participantOptions.map((p) => (
            <MenuItem key={p.value || '_empty'}>
              <button
                type="button"
                onClick={() => handleSelectParticipant(p.value)}
                className={`group flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm ${
                  p.value === ALL_PARTICIPANTS_VALUE
                    ? 'text-purple-700 data-focus:bg-purple-50 data-focus:text-purple-800'
                    : 'text-gray-700 data-focus:bg-blue-50 data-focus:text-blue-700'
                }`}
              >
                <svg
                  className={`w-4 h-4 ${
                    p.value === ALL_PARTICIPANTS_VALUE
                      ? 'text-purple-400 group-data-focus:text-purple-600'
                      : 'text-gray-400 group-data-focus:text-blue-500'
                  }`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
